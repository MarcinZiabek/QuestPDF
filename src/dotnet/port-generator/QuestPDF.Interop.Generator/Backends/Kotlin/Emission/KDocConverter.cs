...
/// into KDoc: summary and remarks become body text, <c>&lt;param&gt;</c> becomes
/// <c>@param</c>, <c>&lt;returns&gt;</c> becomes <c>@return</c>, and common
/// inline tags become Markdown.
/// </summary>
public static class KDocConverter
{
    /// <summary>Returns KDoc lines (without comment markers), or empty when there is no usable content.</summary>
    public static IReadOnlyList<string> ToKDocLines(string? rawMemberXml)
    {
        ...
        if (returns.Length > 0)
            tags.Add($"@return {CollapseForTag(returns)}");
        ...
        switch (element.Name.LocalName)
        {
            ...
            case "see" or "seealso":
            {
                ...
                else
                {
                    ...
                    sb.Append('[').Append(CrefDisplayName(cref)).Append(']');
                }
                break;
            }
