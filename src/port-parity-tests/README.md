...
The JVM (Maven) and TypeScript (npm) QuestPDF ports are thin fluent-API and
...
(`src/dotnet/library/QuestPDF.DocumentationExamples`) are ported to Kotlin,
Java (the JVM package serves both languages) and TypeScript, all suites run
against the **same engine version** (the QuestPDF package pinned by
`src/dotnet/interop`), and every produced image must be **byte-identical**
across runtimes.
...
4. runs the Kotlin suite (`gradlew docExamplesTest`), the Java suite
   (`gradlew docExamplesJavaTest`) and the TypeScript suite
...
Write the .NET example as usual, then port it to all three languages following
